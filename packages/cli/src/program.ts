...
/**
 * Build the CLI's command/option tree.
 *
 * Lives apart from the bin entry so tests can import the REAL grammar (and
 * replace the action handlers) instead of maintaining a second copy that
...
export function createProgram(): Command {
  ...
  program
    .command("init")
    .description(
      "Detect the schema source, write schemat.config.json, and take an initial snapshot.",
    )
  ...
  program
    ...
    .action(async (opts: { root: string; port: string; source?: string }) => {
      await runDev({
        root: opts.root,
        // Kept as the raw string: runDev validates it and prints an actionable
        // message, rather than letting node throw a raw ERR_SOCKET_BAD_PORT.
        port: opts.port,
        source: opts.source,
      });
    });
