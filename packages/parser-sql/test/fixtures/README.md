...
- PostgreSQL columns legitimately named `key` or `index` (both UNRESERVED keywords), which must survive with their type, flags, and any inline `REFERENCES`. Index detection is shape-aware: `KEY idx (col)` is an index, `key text NOT NULL` is a column.
